...
expert_packet_init(void) {
    ...
    static hf_register_info hf[] = {
            {&hf_expert_msg,
                    {"Message", "_au.expert.message", FT_STRING, BASE_NONE, NULL, 0, "Aurora expert information", HFILL}
            },
//		{ &hf_expert_group,
//			{ "Group", "_ws.expert.group", FT_UINT32, BASE_NONE, VALS(expert_group_vals), 0, "Aurora expert group", HFILL }
//		},
//		{ &hf_expert_severity,
//			{ "Severity level", "_ws.expert.severity", FT_UINT32, BASE_NONE, VALS(expert_severity_vals), 0, "Aurora expert severity level", HFILL }
//		}
    };
    static gint *ett[] = {
            &ett_expert,
    ...
    if (proto_expert == -1) {
        proto_expert = proto_register_protocol("Expert Info", "Expert", "_au.expert");
...
expert_register_field_init(expert_field_info *expinfo, expert_module_t *module) {
    ...
    if (gpa_expertinfo.len >= gpa_expertinfo.allocated_len) {
        if (!gpa_expertinfo.ei) {
            ...
            gpa_expertinfo.ei = (expert_field_info **) g_malloc(
                    sizeof(expert_field_info *) * PRE_ALLOC_EXPERT_FIELDS_MEM);
        } else {
            gpa_expertinfo.allocated_len += 1000;
            gpa_expertinfo.ei = (expert_field_info **) g_realloc(gpa_expertinfo.ei,
                                                                 sizeof(expert_field_info *) *
                                                                 gpa_expertinfo.allocated_len);
        }
    }
...
static void
expert_set_info_vformat(packet_info *pinfo, proto_item *pi, int group, int severity, int hf_index,
...
expert_add_info_internal(packet_info *pinfo, proto_item *pi, expert_field *expindex, ...) {
    ...
    expert_set_info_vformat(pinfo, pi, eiinfo->group, eiinfo->severity, *eiinfo->hf_info.p_id, FALSE, eiinfo->summary,
                            unused);
    va_end(unused);
}
...
proto_tree_add_expert_internal(proto_tree *tree, packet_info *pinfo, expert_field *expindex,
    ...
    expert_set_info_vformat(pinfo, ti, eiinfo->group, eiinfo->severity, *eiinfo->hf_info.p_id, FALSE, eiinfo->summary,
                            unused);
    va_end(unused);
