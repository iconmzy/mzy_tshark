...
proto_register_gre(void)
{
    static hf_register_info hf[] = {
        ...
        { &hf_gre_flags_strict_source_route,
          { "Strict Source Route Bit", "gre_flags_strict_source_route",
            FT_BOOLEAN, 16, TFS(&tfs_yes_no), GRE_STRICTSOURCE,
            NULL, HFILL }
        },
        { &hf_gre_flags_recursion_control,
          { "Recursion control", "gre_flags_recursion_control",
            FT_UINT16, BASE_DEC, NULL, GRE_RECURSION,
            NULL, HFILL }
        },
        { &hf_gre_flags_ack,
          { "Acknowledgment", "gre_flags_ack",
        ...
        { &hf_gre_flags_reserved,
          { "Flags (Reserved)", "gre_flags_reserved",
        ...
        { &hf_gre_checksum_status,
          { "Checksum Status", "gre_checksum_status",
        ...
        { &hf_gre_key_payload_length,
          { "Payload Length", "gre_key_payload_length",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            "Size of the payload, not including the GRE header", HFILL }
        },
        { &hf_gre_key_call_id,
          { "Call ID", "gre_key_call_id",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            "Contains the Peer's Call ID for the session to which this packet belongs_", HFILL }
        },
        { &hf_gre_sequence_number,
          { "Sequence Number", "gre_sequence_number",
            FT_UINT32, BASE_DEC, NULL, 0x0,
            "The Sequence Number field contains an unsigned 32 bit integer which is inserted by the encapsulator", HFILL }
        },
        { &hf_gre_ack_number,
          { "Acknowledgment Number", "gre_ack_number",
            FT_UINT32, BASE_DEC, NULL, 0x0,
        ...
        { &hf_gre_routing_address_family,
          { "Address Family", "gre_routing_address_family",
            FT_UINT16, BASE_DEC, NULL, 0x0,
