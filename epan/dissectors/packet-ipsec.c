...
void esp_sa_record_add_from_dissector(guint8 protocol, const gchar *srcIP, const char *dstIP,
    ...
    if (extra_esp_sa_records.num_records == 0) {
        extra_esp_sa_records.records = (uat_esp_sa_record_t *) g_malloc(
                sizeof(uat_esp_sa_record_t) * MAX_EXTRA_SA_RECORDS);
    }
        ...
        while ((cpt_suffix < IPSEC_STRLEN_IPV6) && (ipv6_len - cpt - 1 >= 0) && (found == FALSE)) {
            if (ipv6_address[ipv6_len - cpt - 1] == ':') {
                ...
                if (ipv6_len - cpt - 1 == 0) {
                    /* Found a suffix */
                    found = TRUE;
                } else if (ipv6_address[ipv6_len - cpt - 2] == ':') {
                    /* found a suffix */
                    cpt += 2;
                    found = TRUE;
                } else {
...
export_ipsec_pdu(dissector_handle_t dissector_handle, packet_info *pinfo, tvbuff_t *tvb) {
    if (have_tap_listener(exported_pdu_tap)) {
        exp_pdu_data_t *exp_pdu_data = export_pdu_create_common_tags(pinfo, dissector_handle_get_dissector_name(
                dissector_handle), EXP_PDU_TAG_PROTO_NAME);
...
static gboolean
capture_ah(const guchar *pd, int offset, int len, capture_packet_info_t *cpinfo,
...
*/
static void
dissect_esp_authentication(proto_tree *tree, tvbuff_t *tvb, gint len, gint esp_auth_len,
                           guint8 *authenticator_data_computed,
                           gboolean authentication_ok, gboolean authentication_checking_ok) {
    proto_item *item;
    proto_tree *icv_tree;
    ...
    if (esp_auth_len == 0) {
        ...
        if ((authentication_ok) && (authentication_checking_ok)) {
            ...
            icv_tree = proto_tree_add_subtree_format(tree, tvb, len - esp_auth_len, esp_auth_len,
                                                     ett_esp_icv, NULL,
                                                     "Authentication Data [incorrect, should be 0x%s]",
        ...
        icv_tree = proto_tree_add_subtree(tree, tvb, len - esp_auth_len,
                                          esp_auth_len - (len - tvb_captured_length(tvb)),
    ...
    if (g_esp_enable_encryption_decode || g_esp_enable_authentication_check) {
        ...
        */
                        ...
                        if (md_len < 1 || md_len < esp_auth_len) {
                            ...
                            if (authenticator_data_computed_md == 0) {
                                ...
                                if (memcmp(authenticator_data_computed_md, authenticator_data, esp_auth_len)) {
                                    ...
                                    authenticator_data_computed = (guint8 *) wmem_alloc(wmem_packet_scope(),
                                                                                        esp_auth_len * 2 + 1);
            ...
            if (g_esp_enable_encryption_decode) {
                ...
                switch (esp_crypt_algo) {
                    case IPSEC_ENCRYPT_3DES_CBC :
                        ...
                        if (decrypted_len <= 0)
                            ...
                            if (esp_crypt_key_len != gcry_cipher_get_algo_keylen(crypt_algo_libgcrypt)) {
                                fprintf(stderr,
                                        "<ESP Preferences> Error in Encryption Algorithm 3DES-CBC : Bad Keylen (got %u Bits, need %lu)\n",
                                        esp_crypt_key_len * 8,
                    ...
                    case IPSEC_ENCRYPT_CAST5_CBC :
                        ...
                        if (decrypted_len <= 0)
                            ...
                            switch (esp_crypt_key_len * 8) {
                                ...
                                default:
                                    fprintf(stderr,
                                            "<ESP Preferences> Error in Encryption Algorithm CAST5-CBC : Bad Keylen (%u Bits)\n",
                                            esp_crypt_key_len * 8);
                                    decrypt_ok = FALSE;
                            }
                        }
                    ...
                    case IPSEC_ENCRYPT_DES_CBC :
                        ...
                        if (decrypted_len <= 0)
                            ...
                            if (esp_crypt_key_len != gcry_cipher_get_algo_keylen(crypt_algo_libgcrypt)) {
                                fprintf(stderr,
                                        "<ESP Preferences> Error in Encryption Algorithm DES-CBC : Bad Keylen (%u Bits, need %lu)\n",
                                        esp_crypt_key_len * 8,
                                        (unsigned long) gcry_cipher_get_algo_keylen(crypt_algo_libgcrypt) * 8);
                                decrypt_ok = FALSE;
                            } else
                    ...
                    case IPSEC_ENCRYPT_AES_GCM :
                        ...
                        if (decrypted_len <= 0)
                            ...
                            switch (esp_crypt_key_len * 8) {
                                ...
                                default:
                                    fprintf(stderr,
                                            "<ESP Preferences> Error in Encryption Algorithm AES-CTR / AES-GCM : Bad Keylen (%u Bits)\n",
                                            esp_crypt_key_len * 8);
                                    decrypt_ok = FALSE;
                            }
                        }
                        ...
                        */
                        ...
                        if (decrypted_len <= 0)
                            ...
                            switch (esp_crypt_key_len * 8) {
                                ...
                                default:
                                    fprintf(stderr,
                                            "<ESP Preferences> Error in Encryption Algorithm TWOFISH-CBC : Bad Keylen (%u Bits)\n",
                                            esp_crypt_key_len * 8);
                                    decrypt_ok = FALSE;
                            }
                        }
                    ...
                    case IPSEC_ENCRYPT_BLOWFISH_CBC :
                        ...
                        */
                        ...
                        if (decrypted_len <= 0)
                            ...
                            if (esp_crypt_key_len != gcry_cipher_get_algo_keylen(crypt_algo_libgcrypt)) {
                                fprintf(stderr,
                                        "<ESP Preferences> Error in Encryption Algorithm BLOWFISH-CBC : Bad Keylen (%u Bits, need %lu)\n",
                                        esp_crypt_key_len * 8,
                                        (unsigned long) gcry_cipher_get_algo_keylen(crypt_algo_libgcrypt) * 8);
                                decrypt_ok = FALSE;
                            } else
                ...
                if (decrypt_using_libgcrypt) {
                    ...
                    if (!(*cipher_hd_created)) {
                        ...
                        if (err) {
                            ...
                            if (*cipher_hd_created == FALSE) {
                                ...
                                if (err) {
                                    fprintf(stderr,
                                            "<IPsec/ESP Dissector> Error in Algorithm %s Mode %d, gcry_cipher_setkey(key_len=%u) failed: %s\n",
                                            gcry_cipher_algo_name(crypt_algo_libgcrypt), crypt_mode_libgcrypt,
                                            esp_crypt_key_len, gcry_strerror(err));
                    ...
                    if (crypt_mode_libgcrypt == GCRY_CIPHER_MODE_CTR) {
                        ...
                        if (!err) {
                            err = gcry_cipher_decrypt(*cipher_hd, decrypted_data + esp_iv_len, decrypted_len_alloc,
                                                      NULL, 0);
                        }
                    } else {
                        err = gcry_cipher_decrypt(*cipher_hd, decrypted_data, decrypted_len_alloc + esp_iv_len, NULL,
                                                  0);
                    }
                    ...
                    if (err) {
                        ...
                        if (decrypted_len >= esp_auth_len) {
                            tvb_memcpy(tvb, decrypted_data + decrypted_len - esp_auth_len,
                                       (gint) (NEW_ESP_DATA_SIZE + decrypted_len - esp_auth_len), esp_auth_len);
                        }
        ...
        if (decrypt_ok && (decrypted_len > esp_iv_len)) {
            tvb_decrypted = tvb_new_child_real_data(tvb, (guint8 *) wmem_memdup(pinfo->pool, decrypted_data +
                                                                                             sizeof(guint8) *
                                                                                             esp_iv_len,
            ...
            if (tvb_bytes_exist(tvb_decrypted, decrypted_len - esp_iv_len - esp_auth_len - 2, 2)) {
                ...
                if (decrypted_len - esp_iv_len - esp_auth_len - esp_pad_len - 2 >= 0) {
                    ...
                    encapsulated_protocol = tvb_get_guint8(tvb_decrypted,
                                                           decrypted_len - esp_iv_len - esp_auth_len - 1);
        ...
        if (decrypt_dissect_ok) {
            if (esp_tree) {
                ...
                if (tvb_bytes_exist(tvb, len - 12, 12)) {
                    ...
                    /* Truncated so just display what we have */
                    proto_tree_add_bytes_format(esp_tree, hf_esp_authentication_data, tvb, len - 12,
                                                12 - (len - tvb_captured_length(tvb)),
...
proto_register_ipsec(void) {
    ...
    static ei_register_info ei[] = {
            {&ei_esp_sequence_analysis_wrong_sequence_number, {"esp.sequence-analysis.wrong_sequence_number", PI_SEQUENCE, PI_WARN, "Wrong Sequence Number", EXPFILL}}
    };
...
/*    { IPSEC_AUTH_AES_XCBC_MAC_96, "AES-XCBC-MAC-96 [RFC3566]" }, */
    ...
    static uat_field_t esp_uat_flds[] = {
            ...
            UAT_FLD_VS(uat_esp_sa_records, encryption_algo, "Encryption", esp_encryption_type_vals,
                       "Encryption algorithm"),
            UAT_FLD_CSTRING(uat_esp_sa_records, encryption_key_string, "Encryption Key", "Encryption Key"),
            UAT_FLD_VS(uat_esp_sa_records, authentication_algo, "Authentication", esp_authentication_type_vals,
                       "Authentication algorithm"),
    ...
    esp_sequence_analysis_hash = wmem_map_new_autoreset(wmem_epan_scope(), wmem_file_scope(), g_direct_hash,
                                                        g_direct_equal);
    esp_sequence_analysis_report_hash = wmem_map_new_autoreset(wmem_epan_scope(), wmem_file_scope(), g_direct_hash,
                                                               g_direct_equal);
