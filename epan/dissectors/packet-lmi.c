...
proto_register_lmi(void)
{
    static hf_register_info hf[] = {
        { &hf_lmi_call_ref,
            { "Call reference", "lmi_cmd", FT_UINT8, BASE_HEX, NULL, 0,
                NULL, HFILL }},

        { &hf_lmi_msg_type,
            { "Message Type", "lmi_msg_type", FT_UINT8, BASE_HEX, VALS(msg_type_str), 0,
                NULL, HFILL }},

        { &hf_lmi_inf_ele,
            { "Type", "lmi_inf_ele_type", FT_UINT8, BASE_DEC, VALS(element_type_str), 0,
                "Information Element Type", HFILL }},
        { &hf_lmi_inf_len,
            { "Length", "lmi_inf_ele_len", FT_UINT8, BASE_DEC, NULL, 0,
                "Information Element Length", HFILL }},

        { &hf_lmi_rcd_type,
            { "Record Type", "lmi_ele_rcd_type", FT_UINT8, BASE_DEC, VALS(record_type_str), 0,
                NULL, HFILL }},
        { &hf_lmi_send_seq,
            { "Send Seq", "lmi_send_seq", FT_UINT8, BASE_DEC, NULL, 0,
                "Send Sequence", HFILL }},
        { &hf_lmi_recv_seq,
            { "Recv Seq", "lmi_recv_seq", FT_UINT8, BASE_DEC, NULL, 0,
                "Receive Sequence", HFILL }},
        { &hf_lmi_dlci_high,
            { "DLCI High", "lmi_dlci_hi", FT_UINT8, BASE_DEC, NULL, 0x3f,
                "DLCI High bits", HFILL }},
        { &hf_lmi_dlci_low,
            { "DLCI Low", "lmi_dlci_low", FT_UINT8, BASE_DEC, NULL, 0x78,
                "DLCI Low bits", HFILL }},
        { &hf_lmi_new,
            { "DLCI New", "lmi_dlci_new", FT_UINT8, BASE_DEC, VALS(pvc_status_new_str), 0x08,
                "DLCI New Flag", HFILL }},
        { &hf_lmi_act,
            { "DLCI Active","lmi_dlci_act", FT_UINT8, BASE_DEC, VALS(pvc_status_act_str), 0x02,
