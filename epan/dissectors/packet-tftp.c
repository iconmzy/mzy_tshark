...
static void dissect_tftp_message(tftp_conv_info_t *tftp_info,
    ...
    switch (opcode) {
        ...
        case TFTP_RRQ:  /* Read Request，读文件 */
            i1 = tvb_strsize(tvb, offset);
            proto_tree_add_item_ret_string(tftp_tree, hf_tftp_source_file,
                                           tvb, offset, i1, ENC_ASCII | ENC_NA, wmem_file_scope(),
        ...
        case TFTP_WRQ:  /* Write Request，写文件 */
            i1 = tvb_strsize(tvb, offset);
        ...
        case TFTP_DATA:  /* Data Packet，传输数据包 */
            ...
            is_last_package = (bytes < tftp_info->blocksize);  /* 最后一个数据包小于512字节 */
...
//                    tftp_info->payload_data = (guint8 *) g_try_malloc((gsize) 2000000);

                } else if (blocknum > 1 && tftp_info->payload_data) {
                    guint8 *tmp;
                    tmp = (guint8 *) g_try_malloc((gsize) tftp_info->file_length);
                    memcpy(tmp, tftp_info->payload_data, (tftp_info->next_tap_block_num - 1) * tftp_info->blocksize);
                    tftp_info->payload_data = tmp;
                }
        ...
        case TFTP_ACK:  /* Acknowledgement，应答包 */
...
proto_register_tftp(void) {
    static hf_register_info hf[] = {
            ...
            {&hf_tftp_source_file,
                    {"Source File",                   "tftp_source_file",
                            FT_STRINGZ,  BASE_NONE, NULL,                       0x0,
                            "TFTP source file name",                                  HFILL}},

            {&hf_tftp_destination_file,
                    {"Destination File",              "tftp_destination_file",
                            FT_STRINGZ,  BASE_NONE, NULL,                       0x0,
            ...
            {&hf_tftp_fragments,
                    {"TFTP Fragments",                "tftp_fragments",
                            FT_NONE,     BASE_NONE, NULL,                       0x00,
                            NULL,                                                     HFILL}},

            {&hf_tftp_fragment,
                    {"TFTP Fragment",                 "tftp_fragment",
                            FT_FRAMENUM, BASE_NONE, NULL,                       0x00,
                            NULL,                                                     HFILL}},

            {&hf_tftp_fragment_overlap,
                    {"Fragment overlap",              "tftp_fragment_overlap",
                            FT_BOOLEAN,  0,         NULL,                       0x00,
                            "Fragment overlaps with other fragments",                 HFILL}},

            {&hf_tftp_fragment_overlap_conflicts,
                    {"Conflicting data in fragment overlap",
                                                      "tftp_fragment_overlap_conflicts",
                            FT_BOOLEAN,  0,         NULL,                       0x00,
                            "Overlapping fragments contained conflicting data",       HFILL}},

            {&hf_tftp_fragment_multiple_tails,
                    {"Multiple tail fragments found", "tftp_fragment_multipletails",
                            FT_BOOLEAN,  0,         NULL,                       0x00,
                            "Several tails were found when defragmenting the packet", HFILL}},

            {&hf_tftp_fragment_too_long_fragment,
                    {"Fragment too long",             "tftp_fragment_toolongfragment",
                            FT_BOOLEAN,  0,         NULL,                       0x00,
                            "Fragment contained data past end of packet",             HFILL}},

            {&hf_tftp_fragment_error,
                    {"Defragmentation error",         "tftp_fragment_error",
                            FT_FRAMENUM, BASE_NONE, NULL,                       0x00,
                            "Defragmentation error due to illegal fragments",         HFILL}},

            {&hf_tftp_fragment_count,
                    {"Fragment count",                "tftp_fragment_count",
                            FT_UINT32,   BASE_DEC,  NULL,                       0x00,
                            NULL,                                                     HFILL}},

            {&hf_tftp_reassembled_in,
                    {"Reassembled TFTP in frame",     "tftp_reassembled_in",
                            FT_FRAMENUM, BASE_NONE, NULL,                       0x00,
                            "This TFTP packet is reassembled in this frame",          HFILL}},

            {&hf_tftp_reassembled_length,
                    {"Reassembled TFTP length",       "tftp_reassembled_length",
                            FT_UINT32,   BASE_DEC,  NULL,                       0x00,
                            "The total length of the reassembled payload",            HFILL}},

            {&hf_tftp_reassembled_data,
                    {"Reassembled TFTP data",         "tftp_reassembled_data",
                            FT_BYTES,    BASE_NONE, NULL,                       0x0,
