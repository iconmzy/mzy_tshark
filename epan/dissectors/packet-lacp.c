...
/* Setup list of header fields */
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_lacp_tlv_type,
          { "TLV Type",               "lacp_tlv_type",
            FT_UINT8,    BASE_HEX,    VALS(lacp_type_vals),    0x0,
            NULL, HFILL }},

        { &hf_lacp_tlv_length,
          { "TLV Length",             "lacp_tlv_length",
            FT_UINT8,    BASE_HEX,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_sysid_priority,
          { "Actor System Priority",  "lacp_actor_sys_priority",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_sysid,
          { "Actor System ID",         "lacp_actor_sysid",
            FT_ETHER,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_key,
          { "Actor Key",            "lacp_actor_key",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_port_priority,
          { "Actor Port Priority",            "lacp_actor_port_priority",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_port,
          { "Actor Port",            "lacp_actor_port",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_state,
          { "Actor State",            "lacp_actor_state",
            FT_UINT8,    BASE_HEX,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_actor_state_str,
          { "Actor State Flags",            "lacp_actor_state_str",
            FT_STRING,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_flags_a_activity,
          { "LACP Activity",        "lacp_actor_state_activity",
            FT_BOOLEAN,    8,        TFS(&tfs_active_passive),    LACPDU_FLAGS_ACTIVITY,
            NULL, HFILL }},

        { &hf_lacp_flags_a_timeout,
          { "LACP Timeout",        "lacp_actor_state_timeout",
            FT_BOOLEAN,    8,        TFS(&tfs_short_long_timeout),    LACPDU_FLAGS_TIMEOUT,
            NULL, HFILL }},

        { &hf_lacp_flags_a_aggregation,
          { "Aggregation",        "lacp_actor_state_aggregation",
            FT_BOOLEAN,    8,        TFS(&tfs_aggregatable_individual),    LACPDU_FLAGS_AGGREGATION,
            NULL, HFILL }},

        { &hf_lacp_flags_a_sync,
          { "Synchronization",        "lacp_actor_state_synchronization",
            FT_BOOLEAN,    8,        TFS(&tfs_in_sync_out_sync),    LACPDU_FLAGS_SYNC,
            NULL, HFILL }},

        { &hf_lacp_flags_a_collecting,
          { "Collecting",        "lacp_actor_state_collecting",
            FT_BOOLEAN,    8,        TFS(&tfs_enabled_disabled),    LACPDU_FLAGS_COLLECTING,
            NULL, HFILL }},

        { &hf_lacp_flags_a_distrib,
          { "Distributing",        "lacp_actor_state_distributing",
            FT_BOOLEAN,    8,        TFS(&tfs_enabled_disabled),    LACPDU_FLAGS_DISTRIB,
            NULL, HFILL }},

        { &hf_lacp_flags_a_defaulted,
          { "Defaulted",        "lacp_actor_state_defaulted",
        ...
        { &hf_lacp_flags_a_expired,
          { "Expired",        "lacp_actor_state_expired",
            FT_BOOLEAN,    8,        TFS(&tfs_yes_no),    LACPDU_FLAGS_EXPIRED,
            NULL, HFILL }},

        { &hf_lacp_actor_reserved,
          { "Reserved",        "lacp_actor_reserved",
            FT_BYTES,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_sysid_priority,
          { "Partner System Priority",  "lacp_partner_sys_priority",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_sysid,
          { "Partner System",            "lacp_partner_sysid",
            FT_ETHER,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_key,
          { "Partner Key",            "lacp_partner_key",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_port_priority,
          { "Partner Port Priority",            "lacp_partner_port_priority",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_port,
          { "Partner Port",            "lacp_partner_port",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
        ...
        { &hf_lacp_partner_state,
          { "Partner State",            "lacp_partner_state",
            FT_UINT8,    BASE_HEX,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_partner_state_str,
          { "Partner State Flags",            "lacp_partner_state_str",
            FT_STRING,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_flags_p_activity,
          { "LACP Activity",        "lacp_partner_state_activity",
            FT_BOOLEAN,    8,        TFS(&tfs_active_passive),    LACPDU_FLAGS_ACTIVITY,
            NULL, HFILL }},

        { &hf_lacp_flags_p_timeout,
          { "LACP Timeout",        "lacp_partner_state_timeout",
            FT_BOOLEAN,    8,        TFS(&tfs_short_long_timeout),    LACPDU_FLAGS_TIMEOUT,
            NULL, HFILL }},

        { &hf_lacp_flags_p_aggregation,
          { "Aggregation",        "lacp_partner_state_aggregation",
            FT_BOOLEAN,    8,        TFS(&tfs_aggregatable_individual),    LACPDU_FLAGS_AGGREGATION,
            NULL, HFILL }},

        { &hf_lacp_flags_p_sync,
          { "Synchronization",        "lacp_partner_state_synchronization",
            FT_BOOLEAN,    8,        TFS(&tfs_in_sync_out_sync),    LACPDU_FLAGS_SYNC,
            NULL, HFILL }},

        { &hf_lacp_flags_p_collecting,
          { "Collecting",        "lacp_partner_state_collecting",
            FT_BOOLEAN,    8,        TFS(&tfs_enabled_disabled),    LACPDU_FLAGS_COLLECTING,
            NULL, HFILL }},

        { &hf_lacp_flags_p_distrib,
          { "Distributing",        "lacp_partner_state_distributing",
            FT_BOOLEAN,    8,        TFS(&tfs_enabled_disabled),    LACPDU_FLAGS_DISTRIB,
            NULL, HFILL }},

        { &hf_lacp_flags_p_defaulted,
          { "Defaulted",        "lacp_partner_state_defaulted",
            FT_BOOLEAN,    8,        TFS(&tfs_yes_no),    LACPDU_FLAGS_DEFAULTED,
            "1 = Actor Rx machine is using DEFAULT Partner info, 0 = using info in Rx'd LACPDU", HFILL }},

        { &hf_lacp_flags_p_expired,
          { "Expired",        "lacp_partner_state_expired",
            FT_BOOLEAN,    8,        TFS(&tfs_yes_no),    LACPDU_FLAGS_EXPIRED,
            NULL, HFILL }},

        { &hf_lacp_partner_reserved,
          { "Reserved",        "lacp_partner_reserved",
            FT_BYTES,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},

        { &hf_lacp_coll_max_delay,
          { "Collector Max Delay",  "lacp_collector_max_delay",
            FT_UINT16,    BASE_DEC,    NULL,    0x0,
            "The max delay of the station sending the LACPDU (in tens of usecs)", HFILL }},

        { &hf_lacp_coll_reserved,
          { "Reserved",        "lacp_coll_reserved",
            FT_BYTES,    BASE_NONE,    NULL,    0x0,
            NULL, HFILL }},
