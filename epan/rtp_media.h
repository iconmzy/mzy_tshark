 ...
 */
WS_DLL_PUBLIC  GHashTable *rtp_decoder_hash_table_new(void);
 ...
 */
WS_DLL_PUBLIC size_t decode_rtp_packet_payload(guint8 payload_type, const gchar *payload_type_str, guint8 *payload_data, size_t payload_len, SAMPLE **out_buff, GHashTable *decoders_hash, guint *channels_ptr, guint *sample_rate_ptr);
 ...
 * @return The number of decoded bytes on success, 0 on failure.
 */
WS_DLL_PUBLIC size_t decode_rtp_packet(rtp_packet_t *rp, SAMPLE **out_buff, GHashTable *decoders_hash, guint *channels_ptr, guint *sample_rate_ptr);
