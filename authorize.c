...
void verify_identity_one(const char *reg_path) {
	...
	n = strlen(reg_path);
	for (i = 0; i < n; i++) file[i] = reg_path[i];
	file[n] = 0;
	sprintf(file, "%sactivecode.txt", file);
	...
	for (i = 0; i < n; i++) {
		sum = 0;
		for (j = 0; j < n; j++) if (j != i) sum += temp[j];
		add[i] = sum;
	}
	...
	for (i = 0; i < n; i++) {
		...
		if (a >= 0 && a <= 9) arr[i] = a;
			...
			a %= 100;
			if (a >= 10 && a <= 99) arr[i] = a % 10; else arr[i] = a;
		}
	}
